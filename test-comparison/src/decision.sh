...
if [ -s "$file" ]; then # if the file is not empty
    ...
    curl -v -u $user:$pass --upload-file "$file" http://10.110.4.212:8081/repository/component-test-comparison/hive2/hive-2.3-comparison-"$build_number".csv # Set the path to the test comparison folder in the external repository.
